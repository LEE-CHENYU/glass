...
class Config {
    constructor() {
        ...
        this.defaults = {
            apiUrl: process.env.tars_API_URL || 'http://localhost:9001',
            apiTimeout: 10000,
            
            webUrl: process.env.tars_WEB_URL || 'http://localhost:3000',
    ...
    loadEnvironmentConfig() {
        if (process.env.tars_API_URL) {
            this.config.apiUrl = process.env.tars_API_URL;
            console.log(`[Config] API URL from env: ${this.config.apiUrl}`);
        }
        
        if (process.env.tars_WEB_URL) {
            this.config.webUrl = process.env.tars_WEB_URL;
            console.log(`[Config] Web URL from env: ${this.config.webUrl}`);
        }
        
        if (process.env.tars_API_TIMEOUT) {
            this.config.apiTimeout = parseInt(process.env.tars_API_TIMEOUT);
        }
        
        if (process.env.tars_ENABLE_JWT) {
            this.config.enableJWT = process.env.tars_ENABLE_JWT === 'true';
        }
        
        if (process.env.tars_CACHE_TIMEOUT) {
            this.config.cacheTimeout = parseInt(process.env.tars_CACHE_TIMEOUT);
        }
        
        if (process.env.tars_LOG_LEVEL) {
            this.config.logLevel = process.env.tars_LOG_LEVEL;
        }
        
        if (process.env.tars_DEBUG) {
            this.config.enableDebugLogging = process.env.tars_DEBUG === 'true';
        }
    ...
    getUserConfigPath() {
        const configDir = path.join(os.homedir(), '.tars');
