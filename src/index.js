...
function setupProtocolHandling() {
    ...
    try {
        if (!app.isDefaultProtocolClient('tars')) {
            const success = app.setAsDefaultProtocolClient('tars');
            if (success) {
                console.log('[Protocol] Successfully set as default protocol client for tars://');
            } else {
                console.warn('[Protocol] Failed to set as default protocol client - this may affect deep linking');
            }
        } else {
            console.log('[Protocol] Already registered as default protocol client for tars://');
        }
    } catch (error) {
    ...
    app.on('second-instance', (event, commandLine, workingDirectory) => {
        ...
        for (const arg of commandLine) {
            if (arg && typeof arg === 'string' && arg.startsWith('tars://')) {
...
async function handleCustomUrl(url) {
    try {
        ...
        if (!url || typeof url !== 'string' || !url.startsWith('tars://')) {
...
async function handleFirebaseAuthCallback(params) {
    ...
    try {
        const functionUrl = 'https://us-west1-tars-interview.cloudfunctions.net/tarsAuthCallback';
...
async function startWebStack() {
  ...
  process.env.tars_API_PORT = apiPort.toString();
  process.env.tars_API_URL = `http://localhost:${apiPort}`;
  process.env.tars_WEB_PORT = frontendPort.toString();
  process.env.tars_WEB_URL = `http://localhost:${frontendPort}`;

  console.log(`🌍 Environment variables set:`, {
    tars_API_URL: process.env.tars_API_URL,
    tars_WEB_URL: process.env.tars_WEB_URL
  });
    ...
    try {
        ...
        autoUpdater.on('update-downloaded', (event, releaseNotes, releaseName, date, url) => {
            ...
            dialog.showMessageBox({
                ...
                message: `A new version of Tars (${releaseName}) has been downloaded. It will be installed the next time you launch the application.`,
